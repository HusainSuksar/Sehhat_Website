#!/usr/bin/env python3
"""
Comprehensive Test Data Generator for Umoor Sehhat System
Populates all 9 apps with realistic test data according to specifications:
- 200 Mozes with 200 Aamils
- 652 Medical Students across 200 mozes
- 10 Staff, 2 Admins
- 120 Hospitals across 200 mozes
- 1200 Patients across 120 hospitals from 200 mozes
- 1200 Medical Records for 1200 patients
- 200 Appointments data
- 50 Evaluation forms with analytics
- 50 Survey forms with analytics
- 250 Araz from 1200 patients
...
from mahalshifa.models import (
    ...
    Prescription, LabTest, VitalSigns, MedicalService, HospitalStaff,
...
from araz.models import (
    ...
    ArazAttachment, PetitionAttachment, ArazStatusHistory, PetitionAssignment,
...
def create_comprehensive_test_data():
    """Create comprehensive test data for all apps"""
...
def create_staff():
    ...
    staff = []
    staff_names = [
        ('Omar', 'Al-Hassan'), ('Layla', 'Al-Mahmoud'), ('Khalid', 'Al-Saadi'),
        ('Noor', 'Al-Rashid'), ('Yusuf', 'Al-Zahra'), ('Aisha', 'Al-Hamdan'),
        ('Ibrahim', 'Al-Mansour'), ('Zahra', 'Al-Qahtani'), ('Hassan', 'Al-Sheikh'),
        ('Mariam', 'Al-Otaibi')
    ]
...
def create_mozes_and_aamils():
    """Create 200 Mozes with 200 Aamils"""
    mozes = []
    cities = ['Riyadh', 'Jeddah', 'Mecca', 'Medina', 'Dammam', 'Taif', 'Tabuk', 'Abha']
    
    # Create in batches of 20
    for batch in range(10):  # 10 batches of 20 mozes each
        batch_mozes = []
        for i in range(1, 21):
            moze_num = batch * 20 + i
            ...
            aamil_user, created = User.objects.get_or_create(
                username=f'aamil_{moze_num}',
                defaults={
                    'first_name': f'Aamil{moze_num}',
                    'last_name': f'Al-Moze{moze_num}',
                    'email': f'aamil{moze_num}@umoorsehhat.com',
                    'role': 'aamil',
                    'its_id': generate_its_id(),
                    'is_active': True,
            ...
            moze, created = Moze.objects.get_or_create(
                name=f"Moze {moze_num} - {city}",
                defaults={
                    'location': city,
                    'address': f"Address {moze_num}, {city}, Saudi Arabia",
                    'aamil': aamil_user,
                    'contact_phone': f'+9665{random.randint(10000000, 99999999)}',
                    'contact_email': f'moze{moze_num}@umoorsehhat.com',
            ...
            batch_mozes.append(moze)
        
        mozes.extend(batch_mozes)
        print(f"📦 Completed batch {batch + 1}/10 ({len(mozes)} mozes created)")
...
def create_doctors(mozes):
    ...
    specialties = [
        ...
        'Urology', 'Oncology', 'Emergency Medicine', 'Internal Medicine'
    ]
    
    # Create doctors for each moze
    for i, moze in enumerate(mozes):
        num_doctors = random.randint(2, 5)  # 2-5 doctors per moze
        ...
        if (i + 1) % 20 == 0:
            print(f"📦 Completed {i + 1}/{len(mozes)} mozes for doctors")
    
    return doctors

def create_hospitals_and_departments(mozes):
    """Create 120 hospitals across 200 mozes"""
    hospitals = []
    hospital_types = ['general', 'specialty', 'clinic', 'emergency', 'rehabilitation']
    
    # Create 120 hospitals in batches
    for batch in range(6):  # 6 batches of 20 hospitals each
        batch_hospitals = []
        for i in range(1, 21):
            hospital_num = batch * 20 + i
            moze = random.choice(mozes)
            hospital_type = random.choice(hospital_types)
            
            hospital, created = Hospital.objects.get_or_create(
                name=f"Hospital {hospital_num} - {moze.location}",
                defaults={
                    'description': f"Comprehensive medical facility in {moze.location}",
                    'address': f"Hospital Address {hospital_num}, {moze.location}",
                    'phone': f'+9665{random.randint(10000000, 99999999)}',
                    'email': f'hospital{hospital_num}@umoorsehhat.com',
            ...
            batch_hospitals.append(hospital)
        
        hospitals.extend(batch_hospitals)
        print(f"📦 Completed batch {batch + 1}/6 ({len(hospitals)} hospitals created)")
    
    return hospitals

def create_medical_students(mozes):
    """Create 652 medical students across 200 mozes"""
    students = []
    ...
    # Distribute students across mozes (approximately 3-4 students per moze)
    students_per_moze = 652 // len(mozes)
    remaining_students = 652 % len(mozes)
    
    student_counter = 1
    for i, moze in enumerate(mozes):
        # Calculate students for this moze
        num_students = students_per_moze + (1 if i < remaining_students else 0)
        ...
        if (i + 1) % 20 == 0:
            print(f"📦 Completed {i + 1}/{len(mozes)} mozes for students")
    
    return students

def create_patients(hospitals, mozes):
    """Create 1200 patients across 120 hospitals from 200 mozes"""
    patients = []
    blood_groups = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']
    genders = ['male', 'female']
    
    # Create patients in batches
    for batch in range(12):  # 12 batches of 100 patients each
        batch_patients = []
        for i in range(1, 101):
            patient_num = batch * 100 + i
            hospital = random.choice(hospitals)
            moze = random.choice(mozes)
            
            # Create patient user
            patient_user, created = User.objects.get_or_create(
                username=f'patient_{patient_num}',
                defaults={
                    'first_name': f'Patient{patient_num}',
                    'last_name': f'Medical{patient_num}',
                    'email': f'patient{patient_num}@umoorsehhat.com',
            ...
            patient, created = Patient.objects.get_or_create(
                ...
                defaults={
                    'first_name': f'Patient{patient_num}',
                    'last_name': f'Medical{patient_num}',
                    'arabic_name': f'مريض{patient_num}',
                    ...
                    'email': f'patient{patient_num}@umoorsehhat.com',
                    'address': f"Patient Address {patient_num}, Saudi Arabia",
                    'emergency_contact_name': f'Emergency Contact {patient_num}',
        ...
        print(f"📦 Completed batch {batch + 1}/12 ({len(patients)} patients created)")
    
    return patients

def create_medical_records(patients, doctors):
    """Create 1200 medical records for 1200 patients"""
    medical_records = []
    
    # Create medical records in batches
    for batch in range(12):  # 12 batches of 100 records each
        batch_records = []
        for i in range(100):
            patient_index = batch * 100 + i
            if patient_index < len(patients):
                ...
                batch_records.append(medical_record)
        
        medical_records.extend(batch_records)
        print(f"📦 Completed batch {batch + 1}/12 ({len(medical_records)} medical records created)")
    
    return medical_records

def create_appointments(patients, doctors):
    """Create 200 additional appointments"""
    appointments = []
...
def create_evaluation_data(mozes, doctors):
    """Create 50 evaluation forms with analytics"""
    evaluation_data = []
...
def create_survey_data(mozes, doctors, students):
    """Create 50 survey forms with analytics"""
    survey_data = []
...
def create_araz_data(patients, doctors):
    """Create 250 araz (petitions) from 1200 patients"""
    araz_data = []
    request_types = [
        'consultation', 'prescription', 'follow_up', 'health_check', 'emergency',
        'referral', 'second_opinion', 'home_visit', 'telemedicine', 'laboratory'
    ]
...
def create_student_academic_data(students):
    ...
    course_data = [
        ...
        ('MED106', 'Clinical Skills', 'Practical clinical procedures'),
        ('MED107', 'Medical Ethics', 'Ethical principles in medicine'),
        ('MED108', 'Research Methods', 'Medical research methodology')
    ]
...
def print_summary():
    ...
    print("="*60)
    print("🎉 Comprehensive test data generation completed successfully!")
    print("="*60)
