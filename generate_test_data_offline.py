#!/usr/bin/env python3
"""
Offline Test Data Generator for Umoor Sehhat
    ...
    for i in range(100):
        try:
            ...
            moze = Moze.objects.create(
                ...
                established_date=random_date(date(2010, 1, 1), date(2020, 1, 1)),
                is_active=True,
    ...
    for i, user in enumerate(student_users[:100]):
        try:
            student = Student.objects.create(
                ...
                enrollment_date=random_date(date(2020, 1, 1), date.today()),
                expected_graduation=random_date(date(2024, 1, 1), date(2028, 1, 1)),
            )
            students_created += 1
    ...
    for i in range(100):
        try:
            ...
            patient = Patient.objects.create(
                ...
                date_of_birth=random_date(date(1950, 1, 1), date(2010, 1, 1)),
                gender=gender,
                phone_number=generate_phone(),
    ...
    for i in range(min(100, len(patients))):
        try:
            ...
            record = MedicalRecord.objects.create(
                ...
                visit_date=random_date(date(2023, 1, 1), date.today()),
                follow_up_date=random_date(date.today(), date(2024, 12, 31)),
            )
            records_created += 1
...
def generate_appointments():
    ...
    if not patients or not doctors:
        print_error("No patients or doctors found for appointments")
        return
    ...
    for i in range(min(100, len(patients))):
        try:
            ...
            appointment = Appointment.objects.create(
                patient=patient,
                doctor=doctor,
                appointment_date=random_date(date(2024, 1, 1), date(2024, 12, 31)),
    ...
    for i, title in enumerate(survey_titles):
        try:
            survey = Survey.objects.create(
                ...
                is_active=True,
                start_date=random_date(date(2024, 1, 1), date.today()),
                end_date=random_date(date.today(), date(2024, 12, 31)),
    ...
    for title in album_titles:
        try:
            album = PhotoAlbum.objects.create(
                ...
                event_date=random_date(date(2023, 1, 1), date.today()),
            )
            albums_created += 1
...
def main():
    ...
    print("🏥 UMOOR SEHHAT OFFLINE TEST DATA GENERATOR")
    print("=" * 50)
