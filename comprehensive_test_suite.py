        ...
        for user_type, user_data in self.test_users.items():
            try:
                ...
                if created:
                    ...
                    user.save()
                    self.log_test(f"Create {user_type} user", True, f"Created {user.username}")
                else:
                    self.log_test(f"Create {user_type} user", True, f"User {user.username} already exists")
            except Exception as e:
                self.log_test(f"Create {user_type} user", False, f"Error: {e}", "HIGH")
